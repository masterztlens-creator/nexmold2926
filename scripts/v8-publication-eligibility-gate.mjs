...
const apiKey = requiredEnv("V8_SEARCH_API_KEY");
...
const result = await runV8ArticleRuntime({
  opportunity: {
    query: "plastic injection molding wall thickness design",
  },
  ...
  actor: {
    id: "v8:publication-eligibility-gate",
    role: "SYSTEM",
  },
  ...
  scope: {
    id: "scope:v8:publication-eligibility-gate",
    geography: "GLOBAL",
    industries: [
      "INJECTION_MOLDING",
    ],
  ...
  context: {
    id: "context:v8:publication-eligibility-gate",
    purpose:
  ...
  problem: {
    id: "problem:v8:publication-eligibility-gate",
    question:
...
const compiled = new ContentCompiler(store).compile({
...
const eligibility = evaluator.evaluate({
  compiled,
  scopeId: result.scopeId,
  contextId: result.contextId,
});
...
assertIncludes(
  eligibility.lineage.map((item) => item.type),
  "DECISION",
...
console.log(
  `[V8-PUBLICATION-ELIGIBILITY] acquired=${result.acquisition.length}`,
);
