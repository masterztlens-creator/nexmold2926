...
import type { AggregateType, FoundationRecord, FoundationState, FoundationStore, LineageLink } from "./types.js";
...
function cloneRecord<T>(record: FoundationRecord<T>): FoundationRecord<T> {
  return immutable(deepFreeze({ ...record, lineage: [...record.lineage] }));
}
...
export class InMemoryFoundationStore implements FoundationStore {
  ...
  append<T>(input: Omit<FoundationRecord<T>, "recordId" | "fingerprint" | "recordedAt" | "previousFingerprint"> & { readonly recordedAt?: string }): FoundationRecord<T> {
    ...
    invariant(["SYSTEM", "INGESTOR", "AUDITOR", "VERIFIER"].includes(input.actor.role), "V8_FOUNDATION_AUDIT_ROLE_INVALID", "Audit actor role is invalid.");
    ...
    if (previous) {
      invariant(input.version === previous.version + 1, "V8_FOUNDATION_VERSION_GAP", "Aggregate versions must be contiguous.");
      assertTransition(previous.state, input.state);
    } else {
      invariant(input.version === 1, "V8_FOUNDATION_FIRST_VERSION", "A new aggregate must start at version 1.");
    }
    ...
    const record = immutable({
      recordId: `${input.aggregateType}:${input.aggregateId}:${input.version}`,
      ...body,
      fingerprint,
    }) as FoundationRecord<T>;
    this.records.push(record as FoundationRecord);
    return cloneRecord(record);
  }

  get<T>(aggregateType: AggregateType, aggregateId: string, version?: number): FoundationRecord<T> | null {
    const matches = this.records.filter(r => r.aggregateType === aggregateType && r.aggregateId === aggregateId);
    if (matches.length === 0) return null;
    const record = version === undefined ? matches[matches.length - 1] : matches.find(r => r.version === version);
    return record ? cloneRecord(record as FoundationRecord<T>) : null;
  }

  history(aggregateType: AggregateType, aggregateId: string): readonly FoundationRecord[] {
    return this.records.filter(r => r.aggregateType === aggregateType && r.aggregateId === aggregateId).map(cloneRecord);
  }
  ...
  verifyChain(): void {
    const grouped = new Map<string, FoundationRecord[]>();
    ...
    for (const list of grouped.values()) {
      for (let i = 0; i < list.length; i += 1) {
        const record = list[i];
        const expectedPrevious = i === 0 ? null : list[i - 1].fingerprint;
        invariant(record.previousFingerprint === expectedPrevious, "V8_FOUNDATION_CHAIN_BROKEN", `${record.recordId} previous fingerprint mismatch.`);
        const { fingerprint: actual, ...withoutFingerprint } = record;
