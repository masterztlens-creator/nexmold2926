...
export async function acquireHttpDocument(
  ...
  invariant(
    /^https?:\/\//i.test(url),
    "V8_ACQUIRE_URL_INVALID",
    "Only HTTP(S) sources may be acquired.",
  );
  ...
  const headers = {
    "user-agent":
      options.userAgent ?? "NEXMOLD-V8-Foundation/1.0",
  };
  ...
  for (let hop = 0; hop <= maxRedirects; hop += 1) {
    ...
    if (
      ...
      invariant(
        Boolean(location),
        "V8_ACQUIRE_REDIRECT_WITHOUT_LOCATION",
...
export function persistRawSnapshot(
  bytes: Uint8Array,
  documentHash: string,
  root = ".nexmold/v8/raw-snapshots",
): string {
  invariant(
    /^[a-f0-9]{64}$/i.test(documentHash),
    "V8_SNAPSHOT_HASH_INVALID",
    "Snapshot hash must be SHA-256.",
  );

  const dir = join(
    root,
    documentHash.slice(0, 2),
  );
