...
export class ProjectionGate {
  check(a: PublicationArtifact, p: Projection) {
    invariant(
      p.publicationId === a.id,
      "V8_PROJECTION_PUBLICATION_MISMATCH",
      "Projection must reference the exact publication artifact.",
    );

    invariant(
      p.route.startsWith("/") && p.route !== "/",
      "V8_PROJECTION_ROUTE_INVALID",
      "Projection route must be a non-root path.",
    );

    invariant(
      p.title === a.title && p.body === a.body,
      "V8_PROJECTION_CONTENT_MISMATCH",
      "Projection content does not match publication.",
    );
    ...
    invariant(
      ...
      "Projection fingerprint does not match canonical payload.",
    );

    invariant(
      p.id === `projection:${expected}`,
      "V8_PROJECTION_ID_MISMATCH",
      "Projection id does not match canonical fingerprint.",
    );
