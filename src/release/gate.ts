...
export function assertReleaseReady(r: ReleaseArtifact) {
  ...
  invariant(
    manifest.length === r.manifest.length && manifest.every((x, i) => x === r.manifest[i]),
    "V8_RELEASE_MANIFEST_NOT_CANONICAL",
    "Release manifest is not canonical.",
  );
  invariant(r.projectionId.trim().length > 0, "V8_RELEASE_PROJECTION_REQUIRED", "Release must reference a projection.");
  invariant(r.projectionFingerprint.length === 64, "V8_RELEASE_PROJECTION_FINGERPRINT_INVALID", "Projection fingerprint must be SHA-256.");
  ...
  invariant(
    ...
    "Release fingerprint does not match its canonical payload.",
  );
  invariant(
    r.id === `release:${expected}`,
    "V8_RELEASE_ID_MISMATCH",
    "Release id does not match its canonical fingerprint.",
  );
